...
def main():
    ...
    api_key = 'AIzaSyCGDHY4OJu42Qe9gjwEFzD--ki_9GNZDOo'  
    genai.configure(api_key=api_key)

    # Set up the Streamlit interface
    st.title("Poem Generator")

    # User input for poem theme or description
    user_input = st.text_area(
        "Please describe the theme or mood of the poem (e.g., 'a love poem about nature')",
        height=200
    )
    ...
    if st.button("Generate Poem"):
        if user_input.strip():
            # Create a prompt for generating the poem content
            prompt = f"""
            Based on the following description, please generate a poem:

            "{user_input}"

            The poem should be artistic, expressive, and reflect the theme described.
            """

            try:
                # Use the Gemini generative model to generate the poem content
            ...
            st.warning("Please provide a description of the poem theme or mood.")
